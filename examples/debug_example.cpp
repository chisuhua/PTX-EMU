...
void initialize_debug_environment(const std::string& config_path = "") {
    ...
    if (!config_path.empty()) {
        ...
        if (!loaded) {
            std::cout << "Warning: Failed to load debug configuration file: " << config_path << ". Using default settings.\n";
            // 设置默认调试级别
        ...
        if (!loaded) {
            std::cout << "Warning: Failed to load debug configuration file. Using default settings.\n";
            // 设置默认调试级别
            logger_config.set_global_level(log_level::debug);
        } else {
...
void demonstrate_thread_context_debug() {
    ...
    std::cout << "Breakpoint context prepared with " << bp_context.size() << " entries.\n";
    for (const auto& entry : bp_context) {
...
void demonstrate_perf_timer() {
    ...
    std::cout << "It automatically outputs statistics when going out of scope.\n";
}
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "\nDemo completed. Check the log output and log file (if configured).\n";
