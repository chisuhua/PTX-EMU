...
| # | 债务 | 优先级 | 推荐 change 名 | 工时 |
|---|------|--------|---------------|------|
| D-1 | `docs/README.md` 索引遗漏 9/17 子目录（注：parser-completeness 修复后，docs/README.md 索引 17 个但实际可能仍漏，需 re-verify） | ✅ RESOLVED by `docs-cuda-docs-and-openspec-orphan-sync` (2026-07-06) — 实证 `ls -d docs/*/ \| wc -l` = 16，索引表格覆盖全部 16 个子目录（标题"16 个子目录"） | — | — |
| D-2 | `docs/README.md` 统计数据过时（已修复） | ✅ RESOLVED | — | — |
| D-3 | `docs/skills/README.md` 列 9 vs 实际 18 技能（需 re-verify） | ✅ RESOLVED by `docs-cuda-docs-and-openspec-orphan-sync` (2026-07-06) — 实证 `.opencode/skills/` = 18 个目录（17 active + 1 disabled），docs/skills/README.md 声称"17 active + 1 disabled"，一致 | — | — |
...
| D-5 | `docs/skills/` vs `.opencode/skills/` 内容分叉 | ✅ RESOLVED by `docs-cuda-docs-and-openspec-orphan-sync` (2026-07-06) — 删除 4 个过期/已禁用副本（`ptx-debug/`、`ptxir-serialization/`、`ptx-grammar-modification.md`、`three-mode-testing/`），仅保留 README.md + 2 个非技能技术参考 | — | — |
| D-6 | `HEALTH-AUDIT-2026-06-21.md` 8 个事实错误未合并 | ✅ RESOLVED by `docs-cuda-docs-and-openspec-orphan-sync` (2026-07-06) — 12 个 inline `**[勘误 E1-E8: ...]**` 标记合并到主审计，ERRATA.md 顶部加合并状态标记 | — | — |
