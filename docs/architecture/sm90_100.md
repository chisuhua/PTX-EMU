...
> **范围说明**: 本节描述 Hopper/Blackwell 的执行顺序。
> Tensor Core 指令（`wmma.mma.sync` / `wgmma.async` / `tcgen05.mma`）的实现策略
> 由 [ADR-0016](../adr/0016-blackwell-only-tcgen05.md) 锁定 — **仅实现 Blackwell
> `tcgen05.*`，不实现 pre-Blackwell WMMA**。下文对 ITS / 调度器的描述适用于
> 标量/向量指令；tensor core 指令走独立的 async queue 路径（见设计文档
> `openspec/changes/implement-wmma-tensor-core/`）。
