...
# 9.7.10–9.7.11 纹理与表面指令（Texture and Surface Instructions）详解

> 本节依据 **NVIDIA PTX ISA 9.1**（2025 年发布），系统阐述 **纹理内存（§9.7.10）** 与 **表面内存（§9.7.11）** 指令的完整语法、采样器配置、坐标格式、滤波模式、查询机制及架构支持。这些指令通过专用硬件单元提供高效的缓存、插值、边界处理和随机访问能力，广泛用于图形渲染、图像处理、科学可视化及 GPGPU 输出缓冲。
...
> 📚 **权威参考**：  
> NVIDIA. *PTX: Parallel Thread Execution ISA Version 9.1*.  
> Sections 9.7.10 “Texture Instructions” and 9.7.11 “Surface Instructions”.  
> https://docs.nvidia.com/cuda/parallel-thread-execution/index.html

--- 

此修订版文档已覆盖 PTX ISA 9.1 中纹理与表面指令的全部要素，修正了原始版本中的关键偏差，并补充了遗漏内容（如 `suq`、`.trap` 模式、间接访问等），可作为开发与教学的权威参考。