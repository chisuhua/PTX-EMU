...
## 9.7.3–9.7.5 浮点、半精度与混合精度指令详解

> 本节依据 [NVIDIA 官方 PTX 文档](https://docs.nvidia.com/cuda/parallel-thread-execution/index.html)（PTX ISA 9.1），系统阐述 **浮点指令（9.7.3）**、**半精度与 bfloat16 指令（9.7.4）** 和 **混合精度计算机制（9.7.5）** 的完整语义、数据类型、舍入模式、架构支持及典型用法。所有指令均遵循 IEEE 754 标准（除非明确标注为近似），并适配现代 GPU 架构特性。
