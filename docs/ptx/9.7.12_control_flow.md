...
# 9.7.12 控制流指令（Control Flow Instructions）
> 章节对照：https://docs.nvidia.com/cuda/parallel-thread-execution/index.html#control-flow-instructions  
> 说明：本文件提供**指令索引 + 控制流要点**，完整语义以官方文档为准。

## 快速索引（本节覆盖指令）
- 跳转：`bra`（含 `.uni` 与谓词化）
- 索引跳转：`brx.idx`（配合 `.branchtargets`）
- 调用/返回：`call`, `ret`
- 终止：`exit`
- 调试/异常相关：`trap`, `brkpt`
