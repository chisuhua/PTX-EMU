# ptx-regression-restore Specification

## Purpose

恢复并补全 7 个 PTX 注释单元测试的 ctest 注册（`unit_ptx_integer`、`unit_ptx_float`、
`unit_ptx_extended`、`unit_ptx_bitwise`、`unit_ptx_cvt`、`unit_ptx_ld_st`、`unit_ptx_cvta`），
更新测试代码适配当前 `ptxsim::testing` 命名空间与 `StatementContext` 构造签名，确保
全量编译通过。源自已归档 change `fix-commented-ptx-tests`。
## Requirements
