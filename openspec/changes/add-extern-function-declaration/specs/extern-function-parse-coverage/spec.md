...
为 PTX-EMU parser 的 extern 函数声明解析路径提供 oracle test 覆盖，验证
已存在的双路径实现（PtxListener + PtxVisitor）正确性。

来源：parser-completeness change 的 Metis pre-impl review MR-7 排除项 +
debt audit §A-8 PARTIAL 标记。**本 spec 不引入新功能**，仅覆盖既有实现。
