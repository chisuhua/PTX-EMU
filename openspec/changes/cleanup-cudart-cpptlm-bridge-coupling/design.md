...
After `fix-phase0-gate1-dgpu-bar-leak` (Change 1) merges, `libcudart.so` will still export ~15 CppTLM bridge-related symbols (4 `cpptlm_*` ABI entries + `g_cpptlm_bridge` global + `ptxemu_is/set_bridge_user_override` + 9 `PtxEmuDriverShim` methods). The bridge code path in `cudart_sim.cpp` (~130 lines) and `memory.cpp` (~30 lines) is dead code in the default sync mode and is increasingly fragile (7 conditional branches on `g_cpptlm_bridge`).
...
- **Phase 3 (highest risk)**: Modify CMakeLists.txt (remove `--whole-archive,--exclude-libs=ALL cpptlm_core` link block entirely + `add_subdirectory(CppTLM)`) + delete `PtxEmuDriverShim.{h,cpp}` + `stub_bridge.h` + `cpptlm_bridge.h` + `scripts/regression-cosim.sh` + create `include/cudart/abi_guards.h`. Pass: full regression green. Regenerate baseline (NEW symbols removed: ~14 cpptlm_* + ptxemu_*_override + PtxEmuDriverShim methods + g_cpptlm_bridge; `g_bridge_user_override` is local-anonymous-namespace per Oracle MEDIUM #3, never exported).
