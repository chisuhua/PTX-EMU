# Spec: ptxemu-image-executor

> **能力 ID**: `ptxemu-image-executor`
> **状态**: Proposed(随 `feat-ptxemu-image-executor` change)
> **来源 ADR**: [ADR-0029 §D1-D7](../../adr/ADR-0029-ptxemu-image-executor.md)
> **前置治理**: [ADR-0021 v1.1 amendment](../../adr/ADR-0021-cpptlm-d1-full-integration.md) ✅ merged
> **跨仓集成**: [UsrLinuxEmu ADR-076 §D1/D2](https://example.com/adr-076) canonical for ioctl + HAL details
...
- **Multi-kernel images** (deferred to [ADR-0028 multi-kernel manifest + runtime selection](../../adr/ADR-0029-ptxemu-image-executor.md#d4-v1-单-kernel-per-imageb1单-kernel-v1-保留))
- **Async launch with fence/callback** (deferred to v2; would require `CPPTLM_MODULE_VERSION` bump + new entry points)
- **NVIDIA cubin / fatbin / Tile IR** (explicitly rejected; supported only by real CUDA driver, not by PTX-EMU emulator)
- **Direct CUDA Runtime API** (`cudaLaunchKernel`, `cuModuleLoadData` — those live in the legacy LD_PRELOAD `libcudart.so` path; the executor is an IN-MEMORY backend, NOT a CUDA API replacement)
- **Multi-GPU / multi-process / multi-tenant** (would violate [SINGLE-GPU-INSTANCE] assumptions; explicitly deferred)
- **CP-side integration** (HAL extension in UsrLinuxEmu + IGpuDriver extension in TaskRunner — separate changes per [ADR-076 §Migration Step 2-3](https://example.com/adr-076))

### Out of Scope for this Change (Cross-Repo)

- **UsrLinuxEmu `libptxemu_device.so` consumer** — separate change in UsrLinuxEmu repo per adr-076
- **TaskRunner `cu_module.cpp::cuModuleLoadData` consumer** — separate change in TaskRunner per tadr-307
- **PTX-EMU → UsrLinuxEmu build dependency** — per ADR-0029 §D8.1 key invariant, TaskRunner MUST NOT link `libptxemu_device.so`; the dlsym pattern in `hal_user.cpp` (per adr-076 §D4) handles dynamic loading
