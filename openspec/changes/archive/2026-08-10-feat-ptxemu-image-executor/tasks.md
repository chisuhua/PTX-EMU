...
- [x] 1.1.1 创建 `tests/integration/test_phase0_byte_identical_gates.cpp` 骨架,5 个 test cases
- [x] 1.1.2 失败测试 gate_1_nm_D_libcudart_so_symbol_diff_empty(`nm -D --defined-only libcudart.so | sort > before; ... > after; diff before after`)
- [x] 1.1.3 失败测试 gate_2_soname_libcudart_so_12_preserved(`objdump -p libcudart.so | grep SONAME`)
- [x] 1.1.4 失败测试 gate_3_post_build_symlinks_preserved(`ls -la lib/libcudart.so* | grep -E '\.12$|libcudart\.so$'`)
- [x] 1.1.5 失败测试 gate_4_g_cpptlm_bridge_nullptr_unit_test(单元测试,引用 `if (g_cpptlm_bridge == nullptr)` 路径)
- [x] 1.1.6 失败测试 gate_5_logger_g_gpu_context_clock_path(单元测试,引用 `get_gpu_clock_from_context`,验证搬迁后 logger.cpp:8 extern 仍解析)
- [x] 1.1.7 验证测试 fail:`cmake --build build && ctest -R test_phase0_byte_identical_gates --output-on-failure`(expect 5 failures)

### 1.2 Phase 0 Step 1 实施(Step 3: Implement)

- [x] 1.2.1 **`src/cudart/cpptlm_bridge/PtxEmuDriverShim.cpp`** — 新增 4 bridge 符号定义:
...
- [x] 1.2.2 **`src/cudart/ptx_interpreter.cpp`** — 新增 `g_gpu_context` 定义:
  - `std::unique_ptr<GPUContext> g_gpu_context;`(顶层 TU 全局,与 `PtxInterpreter` 类同 TU)
- [x] 1.2.3 **`src/cudart/cudart_sim.cpp`** — 移除 5 全局符号定义(**仅定义移除,调用点逻辑零修改**):
...
- [x] 1.2.4 验证所有 `g_cpptlm_bridge` / `g_gpu_context` 调用点 **未修改**(grep + diff):
...
- [x] 1.2.5 验证 **logger.cpp:8** `extern size_t get_gpu_clock_from_context()` extern 声明在 `ptx_interpreter.h` 未变(搬迁不破坏解析)

### 1.3 5 gates 验证 pass(Step 4: Verify)

- [x] 1.3.1 gate_1 PASS:`diff <(nm -D --defined-only build/lib/libcudart.so | sort) <(nm -D --defined-only build-baseline/lib/libcudart.so | sort)` 为空(基线 worktree per Lesson §4)
- [x] 1.3.2 gate_2 PASS:`objdump -p build/lib/libcudart.so | grep SONAME` = `libcudart.so.12`
- [x] 1.3.3 gate_3 PASS:`ls -la build/lib/libcudart.so*` 显示 `.12` + 主符号链接
- [x] 1.3.4 gate_4 PASS:`ctest -R "test_cpptlm_bridge_nullptr"` exit 0
- [x] 1.3.5 gate_5 PASS:`ctest -R "test_logger_clock_path"` exit 0(单元测试调用 `get_gpu_clock_from_context()` 返回递增时钟值)
- [x] 1.3.6 额外 baseline 对比:跑现有 230+ ctest 全集,确认 0 regression(per Lesson §14 字节级 fallback 约束)

### 1.4 Commit

- [x] 1.4.1 `git add` Phase 0 Step 1 涉及的所有文件(无 ADR/无 doc 改动)
- [x] 1.4.2 commit message:`refactor(cudart): Phase 0 Step 1 - relocate 5 global symbols per ADR-0029 D2 / ADR-0021 v1.1 amendment. cpptlm_bridge.h ABI unchanged. 5 byte-identical gates verified.`
- [x] 1.4.3 push + 验证 CI 全绿(若适用)
...
- [x] 2.1.1 创建 `tests/unit/cudart/test_cpptlm_module.cpp` 骨架,5 个 test cases + invalid handle 边界
- [x] 2.1.2 失败测试 `ptxemu_image_load_standalone_ptxir_returns_valid_handle`(使用 `tests/ptxir/fixtures/cute_rmsnorm.ptxir` 测试 fixture)
- [x] 2.1.3 失败测试 `ptxemu_image_load_ptxir_embedded_cubin_returns_valid_handle`(使用嵌入 binary fixture)
- [x] 2.1.4 失败测试 `ptxemu_image_load_zero_size_returns_zero`(零字节 image)
- [x] 2.1.5 失败测试 `ptxemu_image_load_corrupt_magic_returns_zero`(前 4 字节 ≠ "PTXI" + 末尾无 `PTXEMB`)
- [x] 2.1.6 失败测试 `ptxemu_image_kernel_name_valid_handle_returns_kernel_string`(验证返回 "cute_rmsnorm_kernel")
- [x] 2.1.7 失败测试 `ptxemu_image_execute_valid_handle_returns_zero`(同步 launch + 验证 grid/block/args 透传)
- [x] 2.1.8 失败测试 `ptxemu_image_execute_zero_handle_returns_einval`(0 handle)
- [x] 2.1.9 失败测试 `ptxemu_image_unload_valid_handle_returns_zero`(正常卸载)
- [x] 2.1.10 失败测试 `ptxemu_image_unload_inflight_returns_ebusy`(执行中调 unload)
- [x] 2.1.11 失败测试 `ptxemu_module_version_returns_CPPTLM_MODULE_VERSION`(返回 1)
- [x] 2.1.12 验证测试 fail:`cmake --build build && ctest -R test_cpptlm_module --output-on-failure`(expect 10 failures)

### 2.2 PtxEmuImageExecutor + cpptlm_module.cpp 实施(Step 3: Implement)

- [x] 2.2.1 **`include/cudart/cpptlm_module.h`** — 新增(per ADR-0029 §D1):
...
- [x] 2.2.2 **`src/cudart/cpptlm_module.cpp`** — `PtxEmuImageExecutor` 类(单例)实现:
  ```cpp
...
- [x] 2.2.4 `execute` 实现(**核心 — D3 mutation bug 修复**):
  - 加 `exec_mu_` lock
...
- [x] 2.2.5 `unload` 实现:
  ...
  - 否则返回 0
- [x] 2.2.6 5 个 `extern "C"` 函数实现(薄 wrapper):
...
- [x] 2.2.7 类头注释包含 7 个 [SINGLE-GPU-INSTANCE] 标记(per ADR-0029 §D6 + ptx-lessons-learned §10):
...
- [x] 2.3.1 **`src/cudart/CMakeLists.txt`** 修改 — 新增 `ptxemu_device` 共享库 target:
  ```cmake
  ...
  ```
- [x] 2.3.2 顶层 `CMakeLists.txt` 修改 — `add_subdirectory(cudart)` 后挂 `ptxemu_device` target(若 cudart CMakeLists 已统一管理)
- [x] 2.3.3 验证 build:
...
- [x] 2.4.1 `tests/unit/cudart/test_cpptlm_module.cpp` 10 个测试 PASS
- [x] 2.4.2 新增 `tests/integration/test_cpptlm_module_dlopen.cpp`:
  - `dlopen("libptxemu_device.so")` 无 libcudart.so 依赖下独立调用
  - 验证 `dlsym` 5 个 ABI 符号 + `ptxemu_module_version() == 1`
  - `dlclose` 清理
- [x] 2.4.3 新增 `tests/integration/test_cpptlm_module_inflight.cpp`:
...
- [x] 2.4.4 新增 `tests/unit/cudart/test_image_executor_mutation.cpp` (D3 修复验证):
...
- [x] 2.4.5 跑全集 ctest 验证 0 regression

### 2.5 Commit

- [x] 2.5.1 `git add` 新增 `include/cudart/cpptlm_module.h` + `src/cudart/cpptlm_module.cpp` + `src/cudart/CMakeLists.txt` 修改 + 测试
- [x] 2.5.2 commit message:`feat(cudart): Phase 1 - cpptlm_module C-API + libptxemu_device.so. 5 ABI entry (load/execute/unload/kernel_name/version), CPPTLM_MODULE_VERSION=1, D3 mutation fix via per-launch re-deserialize. cpptlm_bridge.h unchanged.`
- [x] 2.5.3 push + CI
...
- [x] 3.1.1 新增 `tests/performance/test_ptxir_deserialize_cost.cpp`
- [x] 3.1.2 加载 `bench/cute/cute_rmsnorm.ptx` → 编译为 `cute_rmsnorm.ptxir`(`ptxir_build` 工具,per ADR-0025)
- [x] 3.1.3 测量组 A:`ptxemu_image_load + ptxemu_image_execute × 1`(单次 launch,baseline PtxContext 缓存)
- [x] 3.1.4 测量组 B:`ptxemu_image_load + ptxemu_image_execute × 100`(100 次 launch,每次重 deserialize,per D3)
- [x] 3.1.5 计算 wall time 比 `B/A`
- [x] 3.1.6 阈值断言:比 < 1.10(10% overhead 容差)
- [x] 3.1.7 输出结构化报告:`deserialize_cost=1.05x  PASS` / `deserialize_cost=1.15x  FAIL (触发 A1 fallback)`

### 3.2 perf 结果处理

- [x] 3.2.1 **若 PASS(< 1.10)**:commit perf benchmark + ADR-0029 §合规检查 gate 6 勾选
- [x] 3.2.2 **若 FAIL(≥ 1.10)**:**触发 A1 fallback**(独立 change):
  - 暂停 Commit 3
...
- [x] 3.3.1 `git add` perf benchmark + 结果
- [x] 3.3.2 commit message:`test(perf): D3 deserialize cost gate 6 — cute_rmsnorm wall time ratio 1.05x (PASS < 1.10 threshold). Lock ADR-0029 compliance gate 6.`
...
- [x] 4.1.2 `CHANGELOG.md` 新增 `v0.1.0` entry:
  ```markdown
  ## v0.1.0 (2026-08-XX)
  ...
  ```
- [x] 4.1.3 `docs/dev-process/lessons-learned.md` 新增章节 §44:
...
- [x] 4.1.4 `docs/adr/ADR-0029-ptxemu-image-executor.md` §合规检查 全部勾选:
...
- [x] 4.1.5 `docs/adr/ADR-0021-cpptlm-d1-full-integration.md` §合规检查:
  - [x] ADR-0021 v1.1 amendment merged(v1.1 已 ship)
  - 5 byte-identical gates(若 Phase 0 实施后有 ADR-0021 v1.1 微调需求,触发 v2 amendment)

### 4.2 git tag v0.1.0

- [x] 4.2.1 `git tag -a v0.1.0 -m "feat: PTX-EMU Image Executor (libptxemu_device.so + cpptlm_module.h). ADR-0029 + ADR-0021 v1.1 amendment ship. Cross-repo: UsrLinuxEmu ADR-076 Step 1 complete; TaskRunner tadr-307 unblocked."`
- [x] 4.2.2 `git push --tags`(若已 push 习惯) 或仅本地 tag
- [x] 4.2.3 触发跨仓通知:在 UsrLinuxEmu ADR-076 §Migration Step 2 owner 评审,TaskRunner tadr-307 owner 评审

### 4.3 Commit (docs-only)

- [x] 4.3.1 `git add` README + CHANGELOG + lessons-learned + ADR-0029 + ADR-0021
- [x] 4.3.2 commit message:`docs: PTX-EMU Image Executor v0.1.0 ship — README, CHANGELOG, lessons-learned §44 (D3 mutation pattern), ADR compliance checkboxes`

---

## 5. Final State — ADR-0029 → Accepted

- [x] 5.1 `openspec/changes/feat-ptxemu-image-executor/tasks.md` 全部 checkbox 勾选
