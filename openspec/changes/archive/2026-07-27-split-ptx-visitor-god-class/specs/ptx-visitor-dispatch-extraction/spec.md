# ptx-visitor-dispatch-extraction — Spec

## Purpose

X-Macro dispatch 区 (ptx_visitor.cpp:927-931) 与类别 `#include` 聚合区 (:905-925) 是 visitor 的"注册层"，不包含实际解析逻辑。本 spec 定义提取这两段至独立 `ptx_visitor_dispatch.cpp` 文件的需求。
...
- `include/ptx_ir/ptx_op.def` — X-Macro 指令定义
- `src/ptx_parser/AGENTS.md:48` — 类别分派
