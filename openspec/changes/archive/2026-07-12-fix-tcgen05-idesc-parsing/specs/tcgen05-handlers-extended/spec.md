...
- **AND** the ws path reads the accumulate bit from the idesc register (`instr.operands[3]`) and passes it to the shared `tcgen05_fragment_mma_f16(Tmem&, int warp_id, bool accumulate)` helper (per Oracle C1 fix, 2026-07-11)
- **AND** the helper computes the C slot offset using `c_slot = warp_id * 32 + 64 + lane_id` to support per-warp fragment layouts (single-warp callers passing `warp_id=0` preserve prior layout, per FU-4 forward-compat)
...
- **AND** for `processTcgen05Mma` specifically, T4 (idesc accumulate bit set → `2 × GOLDEN`) and T5 (idesc accumulate bit cleared → `1 × GOLDEN`) test cases SHALL exist per Oracle C1 fix (2026-07-11) — see `openspec/changes/fix-tcgen05-idesc-parsing/specs/tcgen05-idesc-parsing/spec.md` for full semantics
...
- Active predecessor: `archive/2026-07-10-implement-tcgen05-handlers-extended/` (provides helper signature pattern that this change extends)
