...
HSK-8 spec 决策点 1（[decision-1.md §117](../../docs/superpowers/specs/2026-08-22-hsk-8-ptxemu-public-api-ack.md)）承诺 `include/ptx_ir/statement_context.h` 等 IR 类型文件晋升至 `include/ptxemu/ir/`，并提供 `ptxemu::ir` 命名空间包裹。Phase 1 已在 commit `564174f7` 完成 scaffolding（`include/ptxemu/ir/` canonical headers 含完整定义 + ptxemu::ir 命名空间），但实测 **218 个 src/include/tests 文件**仍包含未限定 IR 类型名或使用 `#include "ptx_ir/..."`，未实际迁移（分项以 tasks.md 的可复现 scanner 命令为准；其中 `include/ptx_ir/` 非 shim 头与 `include/ptxir/` 亦在范围内）。HSK-8 audit §Postmortem 明确标记 Phase 1.5 deferred item 触发窗口为"HSK-8 ack 2026-08-22 + 1 release cycle"（≈ 2026-09 中旬），已逼近。
...
- **`src/ptx_ir/*.cpp` 按所属 API 迁移**：canonical IR 实现（如 `ptx_types.cpp`、`operand_context.cpp`、`statement_context.cpp`）迁移至 `ptxemu::ir`；`instruction_latency_table.cpp` 与 `ptx_syntax_utils.cpp` 保留既有 namespace，仅将 IR 类型限定为 `ptxemu::ir::*`
...
- **drift_check workflow 新增 Invariant 8**：使用 token-aware scanner 禁止 caller 文件出现裸 IR 类型名（豁免 3 个 forwarding shim header 与 `include/ptxemu/ir/` canonical 定义头，且忽略 `ptxemu::ir::` 限定、注释和字符串），防止后续回归
...
- `src/ptx_ir/` 7 个实现文件按 design.md 的 namespace ownership 迁移：canonical IR 实现进入 `ptxemu::ir`，其他实现保留其既有 namespace 并限定 IR 类型
...
**工时估算**：Medium (1-2d 累计)，按 Oracle SPLIT 建议拆为 **11-13 个原子 commit**：1.5c+d (1 commit, ~3h) → 1.5e (src/ptx_parser, ~1h) → 1.5f1/f2 (src/ptxsim/instructions + core/utils, ~3h) → 1.5g (src/cudart, ~1h) → 1.5h1/h2 (include/ptx_ir 非 shim + include/ptxir、其他 include, ~2h) → 1.5i1/i2/i3 (tests unit/integration/e2e, ~3h) → 1.5j (gpu_context, ~30min) → 1.5k (drift_check, ~30min)。实际文件集合以 tasks.md 记录的 scanner 输出为准。
