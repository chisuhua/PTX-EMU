...
- [ ] 1.1 更新 `include/ptx_ir/ptxir_format.h`：`PTXIR_VERSION` 2→3，`BARRIER_ENCODED_SIZE` 增加 `sizeof(int32_t)`
- [ ] 1.2 修改 `src/ptx_ir/ptxir_writer.cpp`：`write_barrier()` 追加 `write_i32(out_, instr.reconvergence_pc)`
- [ ] 1.3 修改 `src/ptx_ir/ptxir_reader.cpp`：`read_instruction()` S_BAR case 在 v3 时读取 `reconvergence_pc`（v2 跳过，保持 `-1`）
- [ ] 1.4 修改 `src/ptx_ir/ptxir_reader.cpp`：`read_header()` 版本检查 `if (version_ != 1 && version_ != 2)` 扩展为接受 v3（`version_ != 1 && version_ != 2 && version_ != 3`）
- [ ] 1.5 更新测试 `tests/unit/test_ptxir_serialization.cpp`：
...
- [ ] 1.6 验证：`cd build && cmake -S .. -B . && cmake --build . --target test_ptxir_serialization && ctest -R "unit_ptxir_serialization" --output-on-failure`

## 2. generate_ptxir() 嵌入 CFG 计算

- [ ] 2.1 修改 `src/ptxir/ptxir_serialization.cpp`：`generate_ptxir()` 在 `serialize_statements()` 前插入 CFG 计算逻辑：
  - 构造 `label2pc` 映射
  - 调用 `CFGBuilder::build()` → `CFGBuilder::computePostDominators()`
  - 填充 `kernel->kernelStatements` 中所有 S_BRA 和 S_BAR 的 `reconvergence_pc`
- [ ] 2.2 新增测试 `tests/unit/test_ptxir_serialization.cpp`：
...
- [ ] 2.3 验证：`cd build && cmake --build .` 编译通过
- [ ] 2.4 验证：`ctest --output-on-failure` 全量测试无回归（旧 PTXIR v2 文件兼容）

## 3. 加载路径优化验证

- [ ] 3.1 验证 `load_ptxir()` 默认 `apply_cfg=false` 行为：
  - 确认 v3 PTXIR 文件加载后 S_BRA 和 S_BAR 的 `reconvergence_pc` 正确
  - 确认 `apply_cfg=true` 时旧 v2 文件仍可回退重建
- [ ] 3.2 运行 `./scripts/sanity.sh` 确认无回归
- [ ] 3.3 运行 `./scripts/regression.sh` 确认全量回归通过