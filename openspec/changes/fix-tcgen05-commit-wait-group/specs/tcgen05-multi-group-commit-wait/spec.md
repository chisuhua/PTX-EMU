...
#### Scenario: malformed cta_group IMMEDIATE falls back to 1

- **WHEN** PTX source contains an empty IMMEDIATE (defensive — should not occur in well-formed PTX)
- **THEN** `instr.cta_group == 1u` and no exception is thrown (default safety)
...
#### Scenario: cta_group::2 commit advances group 2 counter

- **WHEN** `processTcgen05Commit` is called with a `Tcgen05Instr{cta_group=2}`
- **THEN** `cta->tc_queue().commit(2)` is invoked
- **AND** the TcQueue counter for group 2 advances independently of group 1
...
#### Scenario: cta_group::2 wait blocks on group 2 counter

- **WHEN** `processTcgen05Wait` is called with a `Tcgen05Instr{cta_group=2}` after a `commit(cta_group=2)` has advanced group 2's counter
- **THEN** `cta->tc_queue().wait(warp, 0, 2)` is invoked
- **AND** the warp returns immediately (counter ≥ 2 satisfied)

#### Scenario: cta_group::2 wait blocks when no group 2 commit has happened

- **WHEN** `processTcgen05Wait` is called with a `Tcgen05Instr{cta_group=2}` but no prior `commit(cta_group=2)` has occurred
- **THEN** the warp blocks on group 2's pending list
- **AND** returns when a subsequent `commit(cta_group=2)` advances the counter
