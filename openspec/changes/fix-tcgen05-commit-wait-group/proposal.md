...
Oracle 2026-07-11 审计（session `ses_0aefd09c3ffeSqBIAGdxiRBFWC`）发现 `tcgen05.commit` 和 `tcgen05.wait` 指令存在 3 个 HIGH-confidence 架构缺陷，**阻碍 FlashAttention 多阶段同步**：
...
2. **`extractQualifiersFromContext`**（`src/ptx_parser/ptx_visitor.cpp:155-183`）走 `TCGEN_CTA_GROUP COLONCOLON IMMEDIATE` 语法树时，遇到 `IMMEDIATE` terminal 调用 `tokenToQualifier` 返回 `Q_UNKNOWN` 后**静默丢弃**。该函数被 **19 个 call sites** 调用（`ptx_visitor.cpp:858`, `_atom.cpp:81`, `_branch.cpp:30`, `_barrier.cpp:86,97`, `_call.cpp:23,44`, `_generic.cpp:14`, `_memory.cpp:16,29,42,55,68`, `_special.cpp:16,24,32,45,58`, `_warp.cpp:24,46`），改其返回类型破坏面过大。
...
**FlashAttention 影响**：FA3 producer-consumer pipeline 需要 QK^T group + softmax group + PV group 区分 commit/wait，硬编码 `group_id=1` 无法表达多阶段同步。`sister-change` `fix-tcgen05-mma-accumulator-and-f32-storage`（Oracle H1+H2 fix，`openspec/changes/fix-tcgen05-mma-accumulator-and-f32-storage/`）修复了 helper 的累加能力，但 B2 测试暴露了"即使累加路径修复，commit/wait 仍无法验证同步边界"。
...
- **修改** `src/ptx_parser/ptx_visitor.cpp::visitTcgen05Inst`：在 `extractQualifiersFromContext(ctx)` 调用后**新增单独 parse tree walk**，从 `tcgen05Qual` 上下文中提取 `TCGEN_CTA_GROUP` 后的 `IMMEDIATE` 值，填充 `instr.cta_group`（这是 Option (b)，Oracle Q5 推荐 — 改 `extractQualifiersFromContext` 返回类型会破坏 19 个 call sites，blash radius 过大）。
...
- **NEW**: 测试 `tests/integration/ptx/test_tcgen05_mma_parse.cpp`：追加 `cta_group::2` 解析验证（`instr.cta_group == 2`）。
...
- **ADR-0018** (`docs/adr/0018-tcgen05-cta-group-restriction.md`)：不变（仍 throw `cta_group::2`）
...
- [x] **Oracle 决策建议**: ✅ 2026-07-11（session `ses_0aefd09c3ffeSqBIAGdxiRBFWC`，Q1+Q2+Q3+Q4+Q5+Q6 全部验证，CONDITIONAL GO）
- [x] **Metis pre-impl review**: 必跑（在 `tasks.md` §0.1）
- [x] **关键假设实证验证**:
...
| `openspec/changes/fix-tcgen05-mma-accumulator-and-f32-storage/` (sister, Oracle H1+H2) | | `fix-tcgen05-ld-st-slot-routing` (FU-3) |
| | | `fix-tcgen05-multi-warp-fragment` (FU-4) |
| | | `tcgen05-flashattention-coverage` (FU-5) |

- **上游**: 已归档的 `implement-tcgen05-handlers-extended` 提供 commit/wait handler 骨架；sister change 提供 helper 累加能力（已 commit 后的 state）
- **下游**: C3 完成后，FU-2 (C1) / FU-3 (C2) / FU-4 (C4) 三个 follow-up 可并行开工（per Oracle Q2），FU-5 (FlashAttention coverage + E2E) 在 FU-1..4 全部完成后
- **本 change 是 FlashAttention-readiness 的第二个前置**（继 sister change H1+H2 之后）
...
- Oracle session: `ses_0aefd09c3ffeSqBIAGdxiRBFWC` (Q1-Q6 验证)
- Oracle sister session: `ses_0b3791d78ffewb52428kJJ2Irz` (原始 H1+H2 审计)
