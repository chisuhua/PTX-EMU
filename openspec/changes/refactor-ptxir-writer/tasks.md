...
- [x] 1.1 MUST 运行 `wc -l src/ptx_ir/ptxir_writer.cpp` 记录基线行数（应为 360）
- [x] 1.2 MUST 运行 `ctest -R ptxir --output-on-failure` 记录基线测试结果（全绿）
- [x] 1.3 MUST 运行 `./tests/ptx/test_all_ptx.sh` 记录基线 PTX 语法测试结果
- [x] 1.4 MUST 生成基线 .ptxir 文件用于二进制一致性对比

## 2. Phase 1: 提取公共 helpers - qualifiers + operands（45 min）

- [x] 2.1 MUST 在 `include/ptx_ir/ptxir_writer.h` 添加 private 方法声明：
...
- [x] 2.2 MUST 在 `ptxir_writer.cpp` 实现 `write_qualifiers()`：提取 qualifier size + 遍历写入逻辑
- [x] 2.3 MUST 在 `ptxir_writer.cpp` 实现 `write_operands()`：提取 operand 遍历 + REG/IMM/other 分发逻辑，`write_imm` 控制是否处理 IMM
- [x] 2.4 MUST 替换 31 个 variant 分支中的 qualifier/operand 遍历循环为 `write_qualifiers()` / `write_operands()` 调用
- [x] 2.5 MUST 验证：`cmake --build build` 通过
- [x] 2.6 MUST 验证：`ctest -R ptxir --output-on-failure` 全绿
- [x] 2.7 MUST 验证：`./tests/ptx/test_all_ptx.sh` 全绿
- [x] 2.8 MUST 验证：新生成的 .ptxir 与基线 byte-identical
- [x] 2.9 git commit -m "refactor(ptxir_writer): extract write_qualifiers/write_operands helpers"

## 3. Phase 2: 提取 per-type helper 方法（60 min）

- [x] 3.1 MUST 在 `include/ptx_ir/ptxir_writer.h` 添加按类别分组的 private helper 声明：
...
- [x] 3.2 MUST 在 `ptxir_writer.cpp` 实现每个 helper 方法（从原 variant 分支体移动，使用 Phase 1 的公共 helpers）
- [x] 3.3 MUST 将 `write_instruction()` 各 `if constexpr` 分支体替换为单行 helper 调用
- [x] 3.4 MUST 验证：`cmake --build build` 通过
- [x] 3.5 MUST 验证：`ctest -R ptxir --output-on-failure` 全绿
- [x] 3.6 MUST 验证：`./tests/ptx/test_all_ptx.sh` 全绿
- [x] 3.7 MUST 验证：新生成的 .ptxir 与基线 byte-identical
- [x] 3.8 git commit -m "refactor(ptxir_writer): extract 31 variant branches to per-type helpers"
