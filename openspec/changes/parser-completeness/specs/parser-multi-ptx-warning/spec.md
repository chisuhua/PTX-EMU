...
修复 PTX-EMU parser 层 multi-PTX cubin 静默截断 bug：当 cubin 包含多个
PTX section 时，`src/ptx_parser/ptx_parser.cpp:60` 的
`ptx_code = of_ptx.str()` 覆盖语义导致只保留最后 section，前 N-1 section
丢失，无任何 warning 告知用户。本 spec 强制恢复累加语义 + 输出
`PTX_WARN_EMU` 警告。

来源：Metis pre-implementation review（2026-07-05）确认此为 12 条债务中
**唯一真 P0 功能缺失**。
