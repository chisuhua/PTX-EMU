...
| tcgen05 handler dispatch | 11 S_TCGEN05_* X-Macro + `Tcgen05PipelineHandler` 3-stage pipeline + `Tcgen05Handler::processTcgen05Operation` (commit `fix-tcgen05-handler-dispatch`)。**9/11 handler 已实现** (commits `486246a` Phase 1 + `178457d` Phase 2 + Phase 1.x)：5 core (mma/ld/st/commit/wait) + 3 alloc-family (alloc/dealloc/relinquish_alloc_permit) + cp。MMA_WS/FENCE 仍抛 `UnsupportedInstructionException`（Phase 3/4 in `implement-tcgen05-handlers-extended`）。cta_group::2 抛清晰异常（ADR-0018）。 |
| tcgen05 测试覆盖 | 5 integration parse 测试 + 1 E2E GEMM kernel + f16×f16→f32 golden value (commit `fix-tcgen05-test-coverage-gaps` fd74261) + 12 TmemAllocator 单元测试 + 12 alloc/dealloc/relinquish handler 集成测试 (commits `486246a` + Phase 1.x) + 7 tcgen05.cp 单元测试 + 3 tcgen05.cp 集成测试 + 1 tcgen05.cp E2E (Priority 3 fallback, ptxas 13.0 不支持 sm_100 cp) (commit `bdb629c`)。tcgen05-tagged ctest 全 PASS,`tests/ptx/test_all_ptx.sh` 45/45。 |
