...
// 等修饰符)
std::vector<ptxemu::ir::Qualifier> cvt_quals(ptxemu::ir::Qualifier dst_dtype, ptxemu::ir::Qualifier src_dtype) {
    return {dst_dtype, src_dtype};
}

// 构造带 .sat 修饰符的 Qualifier 列表
std::vector<ptxemu::ir::Qualifier> cvt_quals_sat(ptxemu::ir::Qualifier dst_dtype, ptxemu::ir::Qualifier src_dtype) {
    return {dst_dtype, src_dtype, ptxemu::ir::Qualifier::Q_SAT};
}

// 构造带指定 rounding modifier 的 Qualifier 列表
std::vector<ptxemu::ir::Qualifier> cvt_quals_round(ptxemu::ir::Qualifier dst_dtype, ptxemu::ir::Qualifier src_dtype,
                                       ptxemu::ir::Qualifier rmod) {
...
TEST_CASE("build_context basic type extraction", "[cvt][context][types]") {
    SECTION("f32 -> s32: src float, dst signed int") {
        auto ctx = build_context(cvt_quals(ptxemu::ir::Qualifier::Q_S32, ptxemu::ir::Qualifier::Q_F32));
    ...
    SECTION("f64 -> f32: both floats, src double") {
        auto ctx = build_context(cvt_quals(ptxemu::ir::Qualifier::Q_F32, ptxemu::ir::Qualifier::Q_F64));
        REQUIRE(ctx.dst_bytes == 4);
    ...
    SECTION("s16 -> s32: both ints, signed extend") {
        auto ctx = build_context(cvt_quals(ptxemu::ir::Qualifier::Q_S32, ptxemu::ir::Qualifier::Q_S16));
        REQUIRE(ctx.dst_bytes == 4);
    ...
    SECTION("u8 -> u64: unsigned widen") {
        auto ctx = build_context(cvt_quals(ptxemu::ir::Qualifier::Q_U64, ptxemu::ir::Qualifier::Q_U8));
...
TEST_CASE("build_context half (f16) is treated as 2-byte float",
          "[cvt][context][half]") {
    SECTION("f16 -> f32") {
        auto ctx = build_context(cvt_quals(ptxemu::ir::Qualifier::Q_F32, ptxemu::ir::Qualifier::Q_F16));
    ...
    SECTION("f32 -> f16") {
        auto ctx = build_context(cvt_quals(ptxemu::ir::Qualifier::Q_F16, ptxemu::ir::Qualifier::Q_F32));
    ...
    SECTION("f16 -> s16 (half to int)") {
        auto ctx = build_context(cvt_quals(ptxemu::ir::Qualifier::Q_S16, ptxemu::ir::Qualifier::Q_F16));
        REQUIRE(ctx.src_bytes == 2);
        REQUIRE(ctx.src_is_float == true);
        REQUIRE(ctx.src_is_half == true);
        REQUIRE(ctx.dst_bytes == 2);
...
TEST_CASE("build_context .sat qualifier", "[cvt][context][sat]") {
    auto ctx = build_context(cvt_quals_sat(ptxemu::ir::Qualifier::Q_U32, ptxemu::ir::Qualifier::Q_F32));
    REQUIRE(ctx.has_sat);
    REQUIRE(ctx.dst_is_float == false);
    REQUIRE(ctx.src_is_float == true);
}
...
TEST_CASE("build_context float rounding qualifiers (.rn/.rz/.rm/.rp/.rna)",
    ...
    SECTION(".rn") {
        auto ctx = build_context(cvt_quals_round(
            ptxemu::ir::Qualifier::Q_S32, ptxemu::ir::Qualifier::Q_F32, ptxemu::ir::Qualifier::Q_RN));
        REQUIRE(ctx.has_rn);
        REQUIRE_FALSE(ctx.has_rni);
    ...
    SECTION(".rz") {
        auto ctx = build_context(cvt_quals_round(
            ptxemu::ir::Qualifier::Q_S32, ptxemu::ir::Qualifier::Q_F32, ptxemu::ir::Qualifier::Q_RZ));
        REQUIRE(ctx.has_rz);
        REQUIRE_FALSE(ctx.has_rn);
    }
    ...
    SECTION(".rm") {
        auto ctx = build_context(cvt_quals_round(
            ptxemu::ir::Qualifier::Q_S32, ptxemu::ir::Qualifier::Q_F32, ptxemu::ir::Qualifier::Q_RM));
        REQUIRE(ctx.has_rm);
    ...
    SECTION(".rp") {
        auto ctx = build_context(cvt_quals_round(
            ptxemu::ir::Qualifier::Q_S32, ptxemu::ir::Qualifier::Q_F32, ptxemu::ir::Qualifier::Q_RP));
        REQUIRE(ctx.has_rp);
    ...
    SECTION(".rna") {
        auto ctx = build_context(cvt_quals_round(
            ptxemu::ir::Qualifier::Q_S32, ptxemu::ir::Qualifier::Q_F32, ptxemu::ir::Qualifier::Q_RNA));
        REQUIRE(ctx.has_rna);
        REQUIRE_FALSE(ctx.has_rn);
    }
}
...
TEST_CASE("build_context integer rounding qualifiers (.rni/.rzi/.rmi/.rpi)",
    ...
    SECTION(".rni") {
        auto ctx = build_context(cvt_quals_round(
            ptxemu::ir::Qualifier::Q_S32, ptxemu::ir::Qualifier::Q_F32, ptxemu::ir::Qualifier::Q_RNI));
        REQUIRE(ctx.has_rni);
        REQUIRE_FALSE(ctx.has_rn);
    }
    ...
    SECTION(".rzi") {
        auto ctx = build_context(cvt_quals_round(
            ptxemu::ir::Qualifier::Q_S32, ptxemu::ir::Qualifier::Q_F32, ptxemu::ir::Qualifier::Q_RZI));
        REQUIRE(ctx.has_rzi);
        REQUIRE_FALSE(ctx.has_rz);
    }
    ...
    SECTION(".rmi") {
        auto ctx = build_context(cvt_quals_round(
            ptxemu::ir::Qualifier::Q_S32, ptxemu::ir::Qualifier::Q_F32, ptxemu::ir::Qualifier::Q_RMI));
        REQUIRE(ctx.has_rmi);
        REQUIRE_FALSE(ctx.has_rm);
    }
    ...
    SECTION(".rpi") {
        auto ctx = build_context(cvt_quals_round(
            ptxemu::ir::Qualifier::Q_S32, ptxemu::ir::Qualifier::Q_F32, ptxemu::ir::Qualifier::Q_RPI));
        REQUIRE(ctx.has_rpi);
        REQUIRE_FALSE(ctx.has_rp);
    }
}
...
TEST_CASE("build_context .rs stochastic rounding",
    ...
    auto ctx = build_context(
        cvt_quals_round(ptxemu::ir::Qualifier::Q_F32, ptxemu::ir::Qualifier::Q_F32, ptxemu::ir::Qualifier::Q_RS));
    REQUIRE(ctx.has_rs);
    REQUIRE_FALSE(ctx.has_rn);
}
...
TEST_CASE("build_context signed/unsigned for various types",
    ...
    SECTION("s8 is signed") {
        auto ctx = build_context(cvt_quals(ptxemu::ir::Qualifier::Q_S8, ptxemu::ir::Qualifier::Q_S8));
    ...
    SECTION("u16 is unsigned") {
        auto ctx = build_context(cvt_quals(ptxemu::ir::Qualifier::Q_U16, ptxemu::ir::Qualifier::Q_U16));
        REQUIRE_FALSE(ctx.dst_is_signed);
        REQUIRE_FALSE(ctx.src_is_signed);
    }

    SECTION("s64 is signed") {
        auto ctx = build_context(cvt_quals(ptxemu::ir::Qualifier::Q_S64, ptxemu::ir::Qualifier::Q_S64));
        REQUIRE(ctx.dst_is_signed);
        REQUIRE(ctx.src_is_signed);
        REQUIRE(ctx.dst_bytes == 8);
    }
    ...
    SECTION("f64 is unsigned (by TypeUtils convention)") {
        ...
        auto ctx = build_context(cvt_quals(ptxemu::ir::Qualifier::Q_F64, ptxemu::ir::Qualifier::Q_F64));
        REQUIRE_FALSE(ctx.dst_is_signed);
        REQUIRE_FALSE(ctx.src_is_signed);
    }
}
...
TEST_CASE("build_context all bytes sizes 1/2/4/8", "[cvt][context][bytes]") {
    // 1-byte
    REQUIRE(
        build_context(cvt_quals(ptxemu::ir::Qualifier::Q_U8, ptxemu::ir::Qualifier::Q_U8)).dst_bytes ==
        1);
    // 2-byte
    REQUIRE(build_context(cvt_quals(ptxemu::ir::Qualifier::Q_U16, ptxemu::ir::Qualifier::Q_U16))
                .dst_bytes == 2);
    // 4-byte
    REQUIRE(build_context(cvt_quals(ptxemu::ir::Qualifier::Q_U32, ptxemu::ir::Qualifier::Q_U32))
                .dst_bytes == 4);
    // 8-byte
    REQUIRE(build_context(cvt_quals(ptxemu::ir::Qualifier::Q_U64, ptxemu::ir::Qualifier::Q_U64))
                .dst_bytes == 8);
}
...
TEST_CASE("build_context signedness by CVT rules",
    ...
    SECTION("s32 -> f32") {
        auto ctx = build_context(cvt_quals(ptxemu::ir::Qualifier::Q_F32, ptxemu::ir::Qualifier::Q_S32));
    ...
    SECTION("u32 -> f32") {
        auto ctx = build_context(cvt_quals(ptxemu::ir::Qualifier::Q_F32, ptxemu::ir::Qualifier::Q_U32));
        REQUIRE(ctx.dst_is_float);
        REQUIRE_FALSE(ctx.dst_is_signed);
        REQUIRE_FALSE(ctx.src_is_signed);
    }
