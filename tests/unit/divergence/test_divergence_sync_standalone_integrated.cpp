...
// "barrier releases all threads" 和 "full warp barrier-then-divergence flow"
// 覆盖。
...
static std::vector<StatementContext> build_divergence_sync_statements() {
    ...
    stmts.push_back(makeBranchInstr(S_BRA, {}, "L_path_a", "%p_lt16", false,
                                    "@%p_lt16 bra L_path_a;"));

    // PC=3: bra L_path_b
    stmts.push_back(
        makeBranchInstr(S_BRA, {}, "L_path_b", "", false, "bra L_path_b;"));
    ...
    // PC=6: bra L_join
    stmts.push_back(
        makeBranchInstr(S_BRA, {}, "L_join", "", false, "bra L_join;"));
    ...
    stmts.push_back(makeBranchInstr(S_BRA, {}, "L_reduce", "%p_t0", false,
                                    "@%p_t0 bra L_reduce;"));

    // PC=15: bra L_exit
    stmts.push_back(
        makeBranchInstr(S_BRA, {}, "L_exit", "", false, "bra L_exit;"));
...
static WarpContext *
create_warp_with_threads(SMContext &sm, std::unique_ptr<CTAContext> block,
...
TEST_CASE("divergence_sync_standalone: barrier releases all threads to "
          "reconvergence PC",
          "[divergence_sync][barrier]") {
    init_factory_once();
    ...
    WarpContext *warp =
        create_warp_with_threads(sm, create_block(statements), register_bank);
...
TEST_CASE("divergence_sync_standalone: thread 0-only branch after barrier",
          "[divergence_sync][barrier][predication]") {
    init_factory_once();
    ResourceManager::instance().initialize(1, 8192);
...
TEST_CASE("divergence_sync_standalone: full warp barrier-then-divergence flow",
          "[divergence_sync][execution][full]") {
    init_factory_once();
    ResourceManager::instance().initialize(1, 8192);
    ...
    // === Per-lane register value assertions (Task 4) ===

    INFO("After ld.shared (PC=17): r3 (= %r_tmp) for all 32 lanes");
    for (int lane = 0; lane < 32; lane++) {
        void *r3_addr = register_bank->get_register("r3", 0, lane);
        REQUIRE(r3_addr != nullptr);
        uint32_t r3_val = *static_cast<uint32_t *>(r3_addr);
        CHECK(r3_val == 0u);
    }

    INFO("After add (PC=18): r4 (= %r_sum) for all 32 lanes");
    for (int lane = 0; lane < 32; lane++) {
        void *r4_addr = register_bank->get_register("r4", 0, lane);
        REQUIRE(r4_addr != nullptr);
        uint32_t r4_val = *static_cast<uint32_t *>(r4_addr);
        CHECK(r4_val == 0u);
    }

    INFO("Verify r1 (= %r_tid) unchanged after Phase 4");
    for (int lane = 0; lane < 32; lane++) {
        void *r1_addr = register_bank->get_register("r1", 0, lane);
        REQUIRE(r1_addr != nullptr);
        uint32_t r1_val = *static_cast<uint32_t *>(r1_addr);
        CHECK(r1_val == static_cast<uint32_t>(lane));
    }
...
TEST_CASE(
    ...
    "[divergence_sync][all_modes][reconvergence]") {
    init_factory_once();
    ResourceManager::instance().initialize(1, 8192);
    ...
    stmts[BRANCH_PC] =
        ptxsim::testing::make_bra_pred("L_reduce", "%p1", false, CONV_PC);
