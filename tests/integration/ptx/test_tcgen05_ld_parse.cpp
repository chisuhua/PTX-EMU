...
TEST_CASE("tcgen05.ld parse → IR",
    ...
    std::vector<ptxemu::ir::OperandContext> operands = {
        ptxemu::ir::OperandContext(RegOperand{"r", 0}),  // dst register r0
        ptxemu::ir::OperandContext(RegOperand{"r", 1}),  // src register r1
    };

    auto stmt = ptxir::factory::makeTcgen05Instr(
        ptxemu::ir::Tcgen05OpKind::LD, qualifiers, operands, ptx_text);

    REQUIRE(std::holds_alternative<ptxemu::ir::Tcgen05Instr>(stmt.data));
    const auto& instr = std::get<ptxemu::ir::Tcgen05Instr>(stmt.data);

    REQUIRE(instr.op_kind == ptxemu::ir::Tcgen05OpKind::LD);
