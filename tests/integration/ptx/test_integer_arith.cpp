// test_integer_arith.cpp
// =============================================================================
// Integration test (类型二) — add.s32 / sub.s32 / mul.s32 on the PTX-EMU
// simulator (NOT real GPU). Drives SMContext + WarpContext + step_warp.
//
// Per TEST_CASE:
//   PC=0:  mov.b32 r1, tid.x    ; r1[lane] = lane_id
//   PC=1:  <arithmetic>         ; see SECTIONs
//   PC=2:  ret
//
...
using ptxsim::testing::make_mul;
using ptxsim::testing::make_ret;
using ptxsim::testing::make_sub;
...
TEST_CASE("integration_ptx_integer_add_s32",
          "[integration][ptx][integer][add]") {
    ...
    for (int lane = 0; lane < 32; ++lane) {
        ...
        uint32_t expected = static_cast<uint32_t>(lane) * 2u;
        CHECK(v == expected);
    }
}

TEST_CASE("integration_ptx_integer_sub_s32",
          "[integration][ptx][integer][sub]") {
    init_instruction_factory_once();
    ResourceManager::instance().initialize(1, 8192);
    ...
    stmts.push_back(make_sub("r2", "r1", "r3")); // PC=2: r2 = r1 - 5
    ...
    for (int lane = 0; lane < 32; ++lane) {
        uint32_t v = get_reg_u32(w, "r2", lane);
        uint32_t expected = static_cast<uint32_t>(lane) - 5u;
        CHECK(v == expected);
    }
}

TEST_CASE("integration_ptx_integer_mul_s32",
          "[integration][ptx][integer][mul]") {
    init_instruction_factory_once();
    ResourceManager::instance().initialize(1, 8192);
