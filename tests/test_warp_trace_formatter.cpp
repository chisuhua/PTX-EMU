...
TEST_CASE("WTF7: format_instruction basic", "[warp_trace_formatter]") {
    ...
    REQUIRE(result == "Cycle 42: SM 0 Warp 3 PC=16  [全部32线程] ld.param");
}

TEST_CASE("WTF8: format_instruction with partial mask", "[warp_trace_formatter]") {
    std::string result = WarpTraceFormatter::format_instruction(
        1, 1, 0, 4, "@%p1 bra", 0xFFFFFFFEu);
    REQUIRE(result == "Cycle 1: SM 1 Warp 0 PC=4  [thread1~31] @%p1 bra");
}
...
TEST_CASE("WTF9: format_simt_push", "[warp_trace_formatter]") {
    ...
    REQUIRE(result.find("taken_mask=0xFFFFFFFE") != std::string::npos);
    REQUIRE(result.find("thread1~31") != std::string::npos);
}
...
TEST_CASE("WTF11: format_divergence with multiple PC groups", "[warp_trace_formatter]") {
    ...
    REQUIRE(result.find("线程分流") != std::string::npos);
    REQUIRE(result.find("thread0→PC=5") != std::string::npos);
    REQUIRE(result.find("thread1~31→PC=8") != std::string::npos);
}
