...
public:
    ...
    void* get_physical_address() override {
        return data_.get();
    }
    ...
    int get_remaining_read_cycles() const override {
        return 0;
    }
    
    int get_remaining_write_cycles() const override {
        return 0;
    }
    ...
    void start_write(const void* buffer, size_t size) override {
        if (size > size_) {
            PTX_DEBUG_EMU("Register write size %zu exceeds register size %zu", size, size_);
            size = size_;
        }
...
public:
    ...
    bool read_register(const std::string& name, void* buffer, size_t size) {
        ...
        memcpy(buffer, reg->get_physical_address(), std::min(size, reg->get_size()));
        return true;
    }
    
    // 写入寄存器值（启动写入过程）
    bool write_register(const std::string& name, const void* buffer, size_t size) {
        auto reg = get_register(name);
        if (!reg) return false;
    ...
    // 获取所有寄存器的名称和接口对
    std::vector<std::pair<std::string, RegisterInterface*>> get_all_registers() const {
...
private:
    std::unordered_map<std::string, std::unique_ptr<RegisterInterface>> registers_;
};
