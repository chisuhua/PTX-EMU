...
public:
    ...
    // 启用/禁用汇聚点跟踪
    void set_trace_convergence_enabled(bool enabled) {
        std::lock_guard<std::mutex> lock(get_mutex());
        trace_convergence_enabled_ = enabled;
    }

    bool is_trace_convergence_enabled() const {
        std::lock_guard<std::mutex> lock(
            const_cast<DebugConfig *>(this)->get_mutex());
        return trace_convergence_enabled_;
    }
...
private:
    ...
    bool trace_convergence_enabled_ = false;  // 汇聚点跟踪开关
