...
struct RegisterInfo {
    ...
    RegisterInfo(const std::string& n, int i, size_t s) : name(n), index(i), size(s) {}
...
public:
    // 分析语句向量，收集所有需要的寄存器信息
    static std::unordered_set<RegisterInfo, RegisterInfoHash> 
    analyze_registers(const std::vector<StatementContext> &statements);

private:
    // 从单个语句中提取寄存器信息
    static void extract_registers_from_statement(const StatementContext &stmt,
                                               std::unordered_set<RegisterInfo, RegisterInfoHash> &registers);

    // 从语句的所有操作数中提取寄存器信息
    static void extract_registers_from_all_operands(const StatementContext &stmt,
                                                   std::unordered_set<RegisterInfo, RegisterInfoHash> &registers);

    // 从操作数中提取寄存器信息
    static void extract_registers_from_operand(const OperandContext &op,
                                             const std::vector<Qualifier> &qualifiers,
                                             std::unordered_set<RegisterInfo, RegisterInfoHash> &registers);

    // 从语句结构体中提取寄存器信息
    template<typename StmtType>
    static void extract_registers_from_struct(const StmtType &stmt,
                                            std::unordered_set<RegisterInfo, RegisterInfoHash> &registers);
};
