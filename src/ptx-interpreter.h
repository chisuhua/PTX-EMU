...
bool trap(int pc) { return breakpoint.count(pc); }
bool run = 0;
bool IFLOG() { return !run && !sync_thread; }
#endif
  ...
  public:
    ...
    void log() {
      ...
      std::printf("INTE: BlockIdx(%d,%d,%d) ThreadIdx(%d,%d,%d)\n", BlockIdx.x,
                  BlockIdx.y, BlockIdx.z, ThreadIdx.x, ThreadIdx.y,
                  ThreadIdx.z);
      std::printf("PC:%d\n", pc);
    }

    // WMMA functions
    void wmma_load(StatementContext::WMMA *ss);
    void wmma_store(StatementContext::WMMA *ss);
    void wmma_mma(StatementContext::WMMA *ss);

    // Helper functions for WMMA
    int getWmmaType(std::vector<Qualifier> &q);
    int getWmmaLayout(std::vector<Qualifier> &q);
    int thread_group_offset(int thrd, int wmma_type, int wmma_layout, int type,
                            int stride);
    int acc_float_offset(int i, int wmma_layout, int stride);
      ...
      for (auto &e : name2Reg) {
        ...
        for (int i = 0; i < ee->elementNum; i++, toti++) {
          ...
          case 'S':
            switch (ee->byteNum) {
            case 2:
              std::printf("%5s%-3d:%-16lx ", ee->name.c_str(), i,
                          *(uint16_t *)((uint64_t)base + i * ee->byteNum));
              break;
            case 4:
              std::printf("%5s%-3d:%-16lx ", ee->name.c_str(), i,
                          *(uint32_t *)((uint64_t)base + i * ee->byteNum));
              break;
            case 8:
              std::printf("%5s%-3d:%-16lx ", ee->name.c_str(), i,
                          *(uint64_t *)((uint64_t)base + i * ee->byteNum));
              break;
            default:
              assert(0);
            }
          ...
          case 'f':
            switch (ee->byteNum) {
            case 4:
              std::printf("%5s%-3d:%-16f ", ee->name.c_str(), i,
                          *(float *)((uint64_t)base + i * ee->byteNum));
              break;
            case 8:
              std::printf("%5s%-3d:%-16lf ", ee->name.c_str(), i,
                          *(double *)((uint64_t)base + i * ee->byteNum));
              break;
            default:
              assert(0);
            }
          ...
          case 'p':
            std::printf("%5s%-3d:%-16d ", ee->name.c_str(), i,
                        *(uint8_t *)((uint64_t)base + i * ee->byteNum));
            break;
          default:
            assert(0);
          }
    ...
    void _exe_once() {
      ...
      case S_WMMA: {
        auto ss = (StatementContext::WMMA *)s.statement;
    ...
    void setIMM(std::string s, Qualifier q) {
      ...
      case Q_F32:
        ...
        s[1] = 'x';
        // when using stoi and facing input 0xBF000000 it will throw
        // std::out_of_range
    ...
    int getBytes(Qualifier q) { return Q2bytes(q); }
    ...
    void *getOperandAddr(OperandContext &op, std::vector<Qualifier> &q) {
      if (op.opType == O_REG) {
        ...
        setIMM(((OperandContext::IMM *)op.operand)->immVal, getDataType(q));
        ...
        if ((*name2Share)[var->varName]) {
          return &(
              (*name2Share)[((OperandContext::VAR *)op.operand)->varName]->val);
...
#ifdef LOGINTE
        if (IFLOG())
          std::printf("INTE: access %s.%s\n", regContext->regMajorName.c_str(),
                      regContext->regMinorName.c_str());
#endif
        static uint64_t t;
    ...
    void *getFaAddr(OperandContext::FA *fa, std::vector<Qualifier> &q) {
      ...
      if (fa->reg) {
        ...
        case 8:
          ret = (void *)*(uint64_t *)ret;
          break;
        case 4:
          ret = (void *)(uint64_t) * (uint32_t *)ret;
          break;
        default:
          assert(0);
        }
    ...
    template <typename T> void _fma(void *to, void *op0, void *op1, void *op2) {
      *(T *)to = std::fma(*(T *)op0, *(T *)op1, *(T *)op2);
    }

    void fma(void *to, void *op0, void *op1, void *op2,
    ...
    template <typename T> void _min(void *to, void *op0, void *op1) {
    ...
    template <typename T> void _max(void *to, void *op0, void *op1) {
    ...
    template <typename T> void _rcp(void *to, void *op) {
      *(T *)to = 1 / *(T *)op;
    }
    ...
    template <typename T> void _or(void *to, void *op0, void *op1) {
      *(T *)to = *(T *)op0 | *(T *)op1;
    }
    ...
    void Or(void *to, void *op0, void *op1, std::vector<Qualifier> &q) {
      ...
      switch (len) {
      case 1:
        _or<uint8_t>(to, op0, op1);
        return;
      case 2:
        _or<uint16_t>(to, op0, op1);
        return;
      case 4:
        _or<uint32_t>(to, op0, op1);
        return;
      case 8:
        _or<uint64_t>(to, op0, op1);
        return;
      default:
        assert(0);
      }
    ...
    template <typename T> void _and(void *to, void *op0, void *op1) {
      *(T *)to = *(T *)op0 & *(T *)op1;
    }
    ...
    void And(void *to, void *op0, void *op1, std::vector<Qualifier> &q) {
      ...
      switch (len) {
      case 1:
        _and<uint8_t>(to, op0, op1);
        return;
      case 2:
        _and<uint16_t>(to, op0, op1);
        return;
      case 4:
        _and<uint32_t>(to, op0, op1);
        return;
      case 8:
        _and<uint64_t>(to, op0, op1);
        return;
      default:
        assert(0);
      }
    ...
    template <typename T> void _xor(void *to, void *op0, void *op1) {
      *(T *)to = *(T *)op0 ^ *(T *)op1;
    }
    ...
    void Xor(void *to, void *op0, void *op1, std::vector<Qualifier> &q) {
      ...
      switch (len) {
      case 1:
        _xor<uint8_t>(to, op0, op1);
        return;
      case 2:
        _xor<uint16_t>(to, op0, op1);
        return;
      case 4:
        _xor<uint32_t>(to, op0, op1);
        return;
      case 8:
        _xor<uint64_t>(to, op0, op1);
        return;
        assert(0);
      }
    }

    template <typename T> void _rem(void *to, void *op0, void *op1) {
      *(T *)to = *(T *)op0 % *(T *)op1;
    }
    ...
    void selp(void *to, void *op0, void *op1, void *pred,
              std::vector<Qualifier> &q) {
      int len = getBytes(q);
      DTYPE dtype = getDType(q);
      switch (len) {
    ...
    template <typename T> void _neg(void *to, void *op0) {
      *(T *)to = -*(T *)op0;
    }
    ...
    template <typename T> void _setp_eq(void *to, void *op0, void *op1) {
    ...
    template <typename T> void _shr(void *to, void *op0, void *op1) {
    ...
    template <typename T> void _shl(void *to, void *op0, void *op1) {
      *(T *)to = *(T *)op0 << *(uint32_t *)op1;
    }
    ...
    void shl(void *to, void *op0, void *op1, std::vector<Qualifier> &q) {
      ...
      switch (len) {
      case 1:
        _shl<uint8_t>(to, op0, op1);
        return;
      case 2:
        _shl<uint16_t>(to, op0, op1);
        return;
      case 4:
        _shl<uint32_t>(to, op0, op1);
        return;
      case 8:
        _shl<uint64_t>(to, op0, op1);
        return;
      default:
        assert(0);
      }
      ...
      case 8: {
        if (dtype[0] == DINT)
          ...
          _cvt<double>(to, from, bitnum[1], dtype[1],
                       QvecHasQ(q, Q_SAT) && dtype[0] == DFLOAT &&
                           dtype[1] == DFLOAT);
        else
          assert(0);
        return;
    ...
    template <typename T> void _div(void *to, void *op0, void *op1) {
      *(T *)to = *(T *)op0 / *(T *)op1;
    }
    ...
    template <typename T> void _mulIntHI(void *to, void *op1, void *op2) {
      __uint128_t t1, t2;
      t1 = *(T *)op1;
      t2 = *(T *)op2;
    ...
    template <typename T> void _mulIntLO(void *to, void *op1, void *op2) {
      *(T *)to = *(T *)op1 * *(T *)op2;
    }

    template <typename T> void _mulfloat(void *to, void *op1, void *op2) {
      *(T *)to = *(T *)op1 * *(T *)op2;
    }
    ...
    void mul(void *to, void *op1, void *op2, std::vector<Qualifier> &q) {
      ...
      case 1:
        switch (mulType) {
        case Q_WIDE:
          _mulIntWide<uint16_t, uint8_t>(to, op1, op2);
          return;
        case Q_HI:
          _mulIntHI<uint8_t>(to, op1, op2);
          return;
        case Q_LO:
          _mulIntLO<uint8_t>(to, op1, op2);
          return;
        }
        return;
      case 2:
        switch (mulType) {
        case Q_WIDE:
          _mulIntWide<uint32_t, uint16_t>(to, op1, op2);
          return;
        case Q_HI:
          _mulIntHI<uint16_t>(to, op1, op2);
          return;
        case Q_LO:
          _mulIntLO<uint16_t>(to, op1, op2);
          return;
        }
        return;
      case 4:
        ...
        case DINT:
          switch (mulType) {
          case Q_WIDE:
            _mulIntWide<uint64_t, uint32_t>(to, op1, op2);
            return;
          case Q_HI:
            _mulIntHI<uint32_t>(to, op1, op2);
            return;
          case Q_LO:
            _mulIntLO<uint32_t>(to, op1, op2);
            return;
          }
        case DFLOAT:
          _mulfloat<float>(to, op1, op2);
          return;
        default:
          assert(0);
        }
      case 8:
        ...
        case DINT:
          switch (mulType) {
          case Q_WIDE:
            _mulIntWide<__uint128_t, uint64_t>(to, op1, op2);
            return;
          case Q_HI:
            _mulIntHI<uint64_t>(to, op1, op2);
            return;
          case Q_LO:
            _mulIntLO<uint64_t>(to, op1, op2);
            return;
          }
        case DFLOAT:
          _mulfloat<double>(to, op1, op2);
          return;
        default:
          assert(0);
        }
    ...
    template <typename T> void _mov(void *from, void *to) {
      *(T *)to = *(T *)from;
    }
