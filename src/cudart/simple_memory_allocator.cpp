...
void SimpleMemoryAllocator::init(size_t size) {
    std::lock_guard<std::mutex> lock(mutex_);

    pool_size_ = size;
    last_allocated_end_ = 0;
...
size_t SimpleMemoryAllocator::allocate(size_t size) {
    ...
    if (free_it == free_blocks_.end()) {
        PTX_DEBUG_MEM("Memory allocation failed: requested size %zu, no "
...
void SimpleMemoryAllocator::deallocate(size_t offset) {
    ...
    if (it != allocated_blocks_.end()) {
        ...
        for (auto free_it = free_blocks_.begin(); free_it != free_blocks_.end();
             ++free_it) {
            size_t free_offset = free_it->second;
            size_t free_size = free_it->first;
...
SimpleMemoryAllocator::Allocation *
SimpleMemoryAllocator::get_allocation(size_t offset) {
    std::lock_guard<std::mutex> lock(mutex_);
