...
param
    : PARAM 
      (globalAlign=ALIGN globalAlignVal=DIGITS)? 
      qualifier 
      (PTR (ptrAlign=ALIGN ptrAlignVal=DIGITS)?)? 
      ID 
      (LeftBracket arrSize=DIGITS RightBracket)?
      ;
...
reg : PERCENT (ID (DOT ID)?) ;

regi : PERCENT (ID (DOT ID)?) ; // reg in vector or fetchAddress
...
fetchAddress : LeftBracket (ID|regi) PLUS? DIGITS? RightBracket ;