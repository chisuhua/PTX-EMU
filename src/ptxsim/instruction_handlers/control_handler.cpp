...
void BraHandler::execute(ThreadContext* context, StatementContext& stmt) {
    auto ss = (StatementContext::BRA*)stmt.statement;
    
    auto iter = context->label2pc.find(ss->braTarget);
    assert(iter != context->label2pc.end());
    context->pc = iter->second - 1; // -1 because pc will be incremented after this instruction
}

void RetHandler::execute(ThreadContext* context, StatementContext& stmt) {
    // 直接设置线程状态为退出
    context->state = EXIT;
}

void BarHandler::execute(ThreadContext* context, StatementContext& stmt) {
