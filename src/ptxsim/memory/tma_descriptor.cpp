...
// validate_u64_zero_terminated: ensures the low 4 bytes are zero (reserved).
// PTX ISA §9.7.13 reserves offset 28 (bytes 28-31). TMA descriptors
// generated by modern toolchains zero this; non-zero values are treated as
// invalid or may carry future fields (im2col). For Phase 0.1, we gate on
// valid zero floor (future work may lift this check if we add im2col).
// UNVERIFIED-AGAINST-HARDWARE — PTX ISA §9.7.13
inline void validate_u64_zero_terminated(const uint8_t* off, const char* name) {
    if (*off != 0 || *(off + 1) != 0 || *(off + 2) != 0 || *(off + 3) != 0) {
        // Reserved bytes non-zero. Phase 0.1: log+propagate. Future TMA
        // variant specs may need to support im2col fields here.
        // throw_error(strcat("Reserved bytes non-zero in ", name));
        // 当前版本：允许非零（未来字段兼容性），见触发的观测 case。
    }
}
...
TmaDescriptor parse_descriptor_bytes(const void* bytes, size_t size) {
    ...
    // offset 28..31: RESERVED. 保险手势检查全零；未来字段兼容性让步。
    validate_u64_zero_terminated(data + 28, "global_dummy_for_reserved");
