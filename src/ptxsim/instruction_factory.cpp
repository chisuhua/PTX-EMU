...
std::unordered_map<StatementType, 
                  std::function<InstructionHandler*()>> InstructionFactory::handler_map;
...
InstructionHandler* InstructionFactory::create_handler(StatementType type) {
    ...
    std::cerr << "No handler registered for statement type: " << static_cast<int>(type) << std::endl;
    return nullptr;
}

void InstructionFactory::register_handler(StatementType type, 
