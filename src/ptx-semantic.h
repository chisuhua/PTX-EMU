...
enum OpType { O_REG, O_VAR, O_IMM, O_VEC, O_FA };
...
public:
  ...
  public:
    ...
    VEC(const VEC &v) { this->vec = v.vec; }
  };
...
public:
  ...
  void exitVector(ptxParser::VectorContext *ctx) override {
    ...
    for (int i = 0; i < ctx->regi().size(); i++) {
      ...
      r->regMinorName =
          ctx->regi(i)->ID(1) ? ctx->regi(i)->ID(1)->getText() : "";
      v->vec.push_back(oc);
    }
    o->operand = v;
    o->opType = O_VEC;
